...
# Bash script with AZ CLI to automate the creation of an Azure Storage account.
...
info() {
    ...
    echo 'storage acct show conn string: '$storage_name
    az storage account show-connection-string \
        --name $storage_name \
        --resource-group $storage_rg \
        --subscription $subscription \
        > out/storage_acct_show_conn_str.json
}
